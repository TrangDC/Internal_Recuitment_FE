...
const ChevronRightStyled = styled(ChevronRight)<CollapseCompact>(({ collapsed, compact }) => ({
  fontSize: 18,
...
const ItemText = styled(Span)<{ compact: any; active: any }>(({ theme, compact, active }) => ({
  whiteSpace: "nowrap",
  paddingLeft: "0.8rem",
  fontSize: "13px",
  fontWeight: 500,
    ...
    <NavExpandRoot className="subMenu">
      <NavItemButton onClick={handleClick} active={hasActive ? 1 : 0}>
        <Box pl="7px" display="flex" alignItems="center">
          ...
          {icon && (
            <item.icon
              sx={{ fontSize: 18, color: hasActive ? "primary.main" : "text.secondary", mr: "4px" }}
            />
          )}
      ...
      <div
        ...
        style={{ maxHeight: !collapsed || sidebarCompact ? "0px" : componentHeight.current + "px" }}
      >
        {children}
      </div>
    </NavExpandRoot>
