...
function CandidatePermissionGroup({ roleTemplate }: PermissionGroupProps) {
      ...
      <CollapseGroup.CollapseBody>
        ...
        <Controller
          ...
          render={({ field }) => (
            <ListCheckBox
              disabled={disabled}
              title={addRemoveBlackAction?.title ?? ''}
              description={addRemoveBlackAction?.description ?? ''}
              for_all={addRemoveBlackAction?.for_all ?? false}
              for_owner={addRemoveBlackAction?.for_owner ?? false}
              for_team={addRemoveBlackAction?.for_team ?? false}
              onCheck={(key) => {
                const data = getCheck(key, field.value, {
                  for_all: addRemoveBlackAction.for_all,
                  for_owner: addRemoveBlackAction.for_owner,
                  for_team: addRemoveBlackAction.for_team,
                })
                field.onChange(data)
              }}
              value={field.value}
...
export default CandidatePermissionGroup
