...
function JobPositionPermissionGroup({ roleTemplate }: PermissionGroupProps) {
  const { control, watch, setValue } = useFormContext()
  const [open, setOpen] = useState(true)
  const createAction = roleTemplate?.JOB_POSITION?.CREATE
  const editAction = roleTemplate?.JOB_POSITION?.EDIT
  const deleteAction = roleTemplate?.JOB_POSITION?.DELETE
  const viewAction = roleTemplate?.JOB_POSITION?.VIEW
    ...
    <CollapseGroup.CollapseContainer
      ...
      title={
        <FlexBox justifyContent={'center'} gap={1}>
          <Text13md color={'grey.900'}>Job position</Text13md>
...
export default JobPositionPermissionGroup
