...
import { JobStatus, TYPE_JOB_STATUS } from 'shared/class/job-status'
import ChipJob from 'shared/class/job-status/components/ChipJob'
import { TYPE_PRIORITY_STATUS, getPriorityByStatus } from 'shared/class/priority'
...
import { CANDIDATE_SOURCE_LABEL, CANDIDATE_SOURCE_STATE } from 'shared/components/autocomplete/candidate-source-auto-complete'
...
import { convertStringToArray, getLastString } from 'shared/utils/convert-string'
...
export function renderDescription(text: string) {
  return <DescriptionRender dangerouslySetInnerHTML={{ __html: text }}></DescriptionRender>
}
...
export function renderReferenceHiringPlatform(text: type_reference_value_Hiring_platform) {
...
export function renderReferenceValue(text: string) {
  ...
  switch(reference_type) {
    ...
    case CANDIDATE_SOURCE_STATE.HIRING_PLATFORM:
      return renderReferenceHiringPlatform(reference_value as type_reference_value_Hiring_platform);
