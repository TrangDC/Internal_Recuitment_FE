...
import checkPermissionColumnsTable from 'features/email/permission/utils/checkPermissionColumnsTable'
...
export const columns = (
  ...
  const columnsTable: ColumnDef<EmailTemplate, any>[] = [
    columnHelper.accessor((row) => row.status, {
      id: 'changeStatusEmail',
      cell: (info) => {
    ...
    columnHelper.accessor('send_to', {
      ...
      size: 200,
      cell: (info) => {
        const send_to = GroupSendToAndRole(
          info.row.original.send_to,
  ...
  const newColumnsTable = checkPermissionColumnsTable({
