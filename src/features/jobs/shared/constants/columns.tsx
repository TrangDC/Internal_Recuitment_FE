...
export const columns = (
  ...
  { me, role }: ParamsColumn
    ...
    columnHelper.accessor((row) => row.name, {
      ...
      cell: (info) => {
        const linkNavigate =
          info.row.original.status === JobStatus.STATUS_HIRING_JOB.PENDING_APPROVALS
            ? `/dashboard/job-detail/${info.row.original.id}`
            : `/dashboard/job-overview/${info.row.original.id}`

        return <LinkText to={linkNavigate}>
          {info.getValue()}
        </LinkText>
      },
      header: () => <span>Name</span>,
